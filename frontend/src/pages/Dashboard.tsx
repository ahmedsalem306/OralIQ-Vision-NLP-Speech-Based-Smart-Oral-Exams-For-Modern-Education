    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", fontFamily: "'Inter',sans-serif" }}>
            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
                <div>
                    <h1 style={{ fontSize: "1.5rem", fontWeight: 800, color: "#e5e5e0", marginBottom: "0.25rem" }}>Exam Questions</h1>
                    <p style={{ color: "#8b8b73", fontSize: "0.875rem" }}>{questions.length} question{questions.length !== 1 ? "s" : ""} — each has a unique shareable link</p>
                </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "1rem" }}>
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} style={{ background: "#141414", border: `1px solid ${G}0.2)`, borderRadius: "1.5rem", padding: "2rem", width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto", boxShadow: `0 0 80px ${G}0.1)` }}>
                            <h2 style={{ fontSize: "1.2rem", fontWeight: 800, color: "#e5e5e0", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                <BookOpen size={20} color="#cfa355" /> New Exam Question
                            </h2>
                            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                                <div><label style={lbl}>Question *</label><textarea value={text} onChange={e => setText(e.target.value)} placeholder="What is the question?" rows={3} style={{ ...inp, resize: "vertical" }} /></div>
                                <div><label style={lbl}>Model Answer *</label><textarea value={modelAnswer} onChange={e => setModelAnswer(e.target.value)} placeholder="The ideal answer..." rows={3} style={{ ...inp, resize: "vertical" }} /></div>
                                <div><label style={lbl}>Keywords (comma-separated)</label><input type="text" value={keywords} onChange={e => setKeywords(e.target.value)} placeholder="e.g. neural network, AI" style={inp} /></div>
                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
                                    <div><label style={lbl}>Category</label><select value={category} onChange={e => setCategory(e.target.value)} style={sel}><option value="general" style={{ background: "#141414" }}>General</option><option value="technical" style={{ background: "#141414" }}>Technical</option><option value="behavioral" style={{ background: "#141414" }}>Behavioral</option><option value="hr" style={{ background: "#141414" }}>HR</option></select></div>
                                ...
                                <div><label style={lbl}>Duration (Minutes)</label><input type="number" min={1} max={10} value={durationMinutes} onChange={e => setDurationMinutes(parseInt(e.target.value) || 2)} style={inp} /></div>
                            </div>
                            <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.5rem" }}>
                                <button onClick={() => { setShowModal(false); resetForm(); }} style={{ flex: 1, padding: "0.8rem", background: "transparent", border: `1px solid ${G}0.12)`, borderRadius: "0.75rem", color: "#8b8b73", fontWeight: 600, cursor: "pointer", fontFamily: "'Inter',sans-serif" }}>Cancel</button>
                                <motion.button whileHover={{ scale: 1.02 }} onClick={handleCreate} disabled={saving || !text.trim() || !modelAnswer.trim()} style={{ flex: 1, padding: "0.8rem", background: "linear-gradient(135deg, #cfa355, #e8c97a)", border: "none", borderRadius: "0.75rem", color: "#0a0a0a", fontWeight: 800, cursor: "pointer", fontFamily: "'Inter',sans-serif", opacity: saving || !text.trim() || !modelAnswer.trim() ? 0.5 : 1 }}>{saving ? "Creating..." : "Create & Get Link"}</motion.button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
            ...
            {loading ? (
                ...
                <div style={{ textAlign: "center", padding: "5rem 2rem", background: "#141414", border: `1px dashed ${G}0.15)`, borderRadius: "1.25rem" }}>
                    <BookOpen size={48} color="#393632" style={{ margin: "0 auto 1rem" }} /><h3 style={{ color: "#8b8b73", fontWeight: 600, marginBottom: "0.5rem" }}>No questions yet</h3><p style={{ color: "#393632", fontSize: "0.875rem" }}>Create your first exam question.</p>
                </div>
            ) : (
                                    ...
                                    {isExp && (
                                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} style={{ overflow: "hidden" }}>
                                            <div style={{ padding: "0 1.5rem 1.25rem", borderTop: `1px solid ${G}0.05)`, paddingTop: "1rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                                                <div><p style={lbl}>Model Answer</p><p style={{ fontSize: "0.875rem", color: "#bbb", lineHeight: 1.7 }}>{q.model_answer}</p></div>
                                                {q.keywords && (<div><p style={lbl}>Keywords</p><div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>{q.keywords.split(",").map((k, i) => (<span key={i} style={{ fontSize: "0.75rem", padding: "0.2rem 0.6rem", background: `${G}0.06)`, border: `1px solid ${G}0.12)`, borderRadius: "999px", color: "#cfa355" }}>{k.trim()}</span>))}</div></div>)}
                                            </div>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: "1rem" }}>
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} style={{ background: "#141414", border: `1px solid ${G}0.2)`, borderRadius: "1.5rem", padding: "2rem", width: "100%", maxWidth: 480, maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: `0 0 60px ${G}0.08)` }}>
                            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.25rem" }}>
                                <div><h2 style={{ fontSize: "1.1rem", fontWeight: 800, color: "#e5e5e0", marginBottom: "0.25rem", display: "flex", alignItems: "center", gap: "0.5rem" }}><UserPlus size={18} color="#cfa355" /> Assign to Student</h2><p style={{ fontSize: "0.8rem", color: "#8b8b73", maxWidth: 340 }}>{assignQuestion.text.slice(0, 80)}{assignQuestion.text.length > 80 ? "..." : ""}</p></div>
                            ...
                            <div style={{ position: "relative", marginBottom: "1rem" }}>
                                ...
                                <input type="text" placeholder="Search students..." value={studentSearch} onChange={e => setStudentSearch(e.target.value)} style={{ ...inp, paddingLeft: "2.25rem" }} />
                            </div>
                            <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                                {filteredStudents.length === 0 ? (<p style={{ textAlign: "center", color: "#8b8b73", padding: "2rem", fontSize: "0.875rem" }}>{!students.length ? "No students found." : "No match."}</p>) : filteredStudents.map(s => {
                                    ...
                                    return (<div key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.85rem 1rem", background: done ? `${G}0.04)` : "rgba(255,255,255,0.015)", border: `1px solid ${done ? `${G}0.2)` : `${G}0.06)`}`, borderRadius: "0.75rem", transition: "all 0.2s" }}>
                                        ...
                                        <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleAssign(s.id)} disabled={assigning || done} style={{ display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.45rem 0.9rem", background: done ? `${G}0.08)` : "linear-gradient(135deg, #cfa355, #e8c97a)", border: done ? `1px solid ${G}0.2)` : "none", borderRadius: "0.6rem", color: done ? "#cfa355" : "#0a0a0a", fontSize: "0.8rem", fontWeight: 700, cursor: done ? "default" : "pointer", fontFamily: "'Inter',sans-serif", opacity: assigning ? 0.6 : 1 }}>{done ? <><Check size={13} /> Assigned</> : "Assign"}</motion.button>
                                    </div>);
                                })}
                            </div>
                            <button onClick={() => setAssignQuestion(null)} style={{ marginTop: "1.25rem", padding: "0.75rem", background: "transparent", border: `1px solid ${G}0.1)`, borderRadius: "0.75rem", color: "#8b8b73", fontWeight: 600, cursor: "pointer", fontFamily: "'Inter',sans-serif" }}>Done</button>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
